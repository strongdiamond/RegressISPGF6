...
from deeplabv3plusModel import DeeplabV3Plus_ResNet50V2
...
def training_deeplabv3plus():

    train_same_prefix_fnames,valid_same_prefix_fnames=get_shuffle_img_gt_train_val_fns(imgs_gts_path,val_samples_num=300)
    ...
    weights_dir = '/demo/GF6_ISP/DeeplabV3Plus_ISP/weights'
    checkpoint_dir='/demo/GF6_ISP/DeeplabV3Plus_ISP/ckpt'
    csv_dir='/demo/GF6_ISP/DeeplabV3Plus_ISP/csv'
    ...
    best_weights_filepath=os.path.join(weights_dir,'isp_deeplabv3plus_gf6_v1.weights.h5') 
    
    model=DeeplabV3Plus_ResNet50V2(image_size=512,num_channels=8,num_classes=1)
    if os.path.exists(best_weights_filepath):
        model.load_weights(best_weights_filepath)
        print('load weights==>'+best_weights_filepath)
        best_weights_filepath=os.path.join(weights_dir,'isp_deeplabv3plus_gf6_v2.weights.h5')
    else:
        pass    
    ...
    csv_logger = CSVLogger(csv_dir+'/isp_deeplabv3plus_gf6_v1.csv', append=True, separator=',')
